...
def load_data():
    """Load social list from the pickle file if it exists, else use defaults."""
    if os.path.exists(DATA_FILE):
        try:
            with open(DATA_FILE, "rb") as f:
                return pickle.load(f)
        except (pickle.UnpicklingError, EOFError):
            print("Save file was unreadable, starting with default data.\n")
            return default_social
    return default_social


def save_data():
    """Dump the current social list to the pickle file."""
